...
_GMC_SPEC_PATH = _Path(__file__).parent / "specs" / "gmc_spec.json"
with open(_GMC_SPEC_PATH, "r", encoding="utf-8") as _f:
    GMC_SPEC = json.load(_f)

def gmc_fields_for_profile(profile: str) -> list[dict]:
    profile = (profile or "general").strip().lower()
    profiles = GMC_SPEC.get("profiles", {})
    fields = profiles.get(profile, [])
    # Map to UI schema used by SPEC_FIELDS client-side
    mapped = []
    for f in fields:
        mapped.append({
            "name": f["name"],
            "description": f.get("desc",""),
            "importance": {"required":"required","conditional":"conditional","recommended":"recommended","optional":"optional"}[f["status"]],
...
def _is_price(v: str) -> bool:
    return bool(re.match(r"^\d+(\.\d{1,2})?\s[A-Z]{3}$", v or ""))

def _is_date(v: str) -> bool:
    return bool(re.match(r"^\d{4}-\d{2}-\d{2}", v or ""))

def _bad_headers(headers: list[str], profile: str) -> list[str]:
    allowed = {f["name"] for f in GMC_SPEC["profiles"].get(profile, [])}
...
def validate_gmc_bytes(data: bytes, delimiter: str, encoding: str, profile: str):
    ...
    # Enforce headers exactly from spec
    profile = (profile or "general").strip().lower()
    bad = _bad_headers(parsed_headers, profile)
    issues = []
    error_rows = set()
    for b in bad:
        _push_issue(issues, error_rows, 0, "", b, "GMC-001", "error", f'Header "{b}" is not allowed for profile "{profile}".', b)
    ...
    for idx, r in enumerate(records):
        ...
        # Price format
        if _norm(r.get("price")) and not _is_price(r.get("price")):
            _push_issue(issues, error_rows, idx, rid, "price", "GMC-200", "error", 'price must be "<amount> <ISO4217>" e.g., "15.00 USD".', r.get("price"))

        # Availability
        av = (r.get("availability","") or "").lower()
        if av and av not in GMC_AVAIL:
            _push_issue(issues, error_rows, idx, rid, "availability", "GMC-210", "error", "availability must be in_stock, out_of_stock, preorder, or backorder.", av)
        if av == "preorder":
            if not _is_date(r.get("availability_date","")):
                _push_issue(issues, error_rows, idx, rid, "availability_date", "GMC-211", "error", "availability_date (ISO 8601) required when availability=preorder.", r.get("availability_date",""))
        ...
        if gtin:
            if not _gtin_checksum_ok(gtin):
                _push_issue(issues, error_rows, idx, rid, "gtin", "GMC-220", "warning", "GTIN checksum appears invalid.", gtin)
        else:
            if not (brand and mpn):
                _push_issue(issues, error_rows, idx, rid, "brand/mpn", "GMC-221", "error", "Provide GTIN or brand + mpn.", f"{brand}|{mpn}")

        # Apparel enums
        if profile == "apparel":
            g = (r.get("gender","") or "").lower()
            if g and g not in GMC_GENDER:
                _push_issue(issues, error_rows, idx, rid, "gender", "GMC-230", "error", "gender must be male/female/unisex.", g)
            ag = (r.get("age_group","") or "").lower()
            if ag and ag not in GMC_AGE:
                _push_issue(issues, error_rows, idx, rid, "age_group", "GMC-231", "error", "age_group must be newborn/infant/toddler/kids/adult.", ag)
    ...
    return {
        ...
        "opportunity_count": len([i for i in issues if i["severity"]=="opportunity"]),
